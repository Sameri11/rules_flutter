...
- Proven plugin shapes are pub plugins with Java/Kotlin Android halves, pub plugins with CMake-built native halves, local path plugins in a monorepo layout, consumer-written Package Recipes, and Dart build-hook packages surfaced through a recipe (native assets). `ndk-build` plugins, prebuilt-JNI plugin shapes, and plugins whose Maven coordinates cannot be read statically are not supported; the last shape requires `plugins.package(artifacts = ...)`.
